...
import zio.test.TestAspect.{ timed, timeout }
import zio.test._
import zio.{ Promise, Ref, UIO, ZIO, ZManaged }
...
object SwitchablePolicySpec extends DefaultRunnableSpec {
  ...
  override def spec = suite("switchable policy")(
    ...
    suite("finish in flight mode")(testM("uses the new policy after switching") {
      val initialPolicy = Retry.make().map(_.toPolicy)
      ...
      val failFirstTime: ZIO[Any, Nothing, ZIO[Any, Unit, Unit]] = for {
        ref   <- Ref.make(0)
        effect = UIO(println("Making failFirstTimeCall")) *> ref
                   .getAndUpdate(_ + 1)
